...
const API_KEY = "live_4ngGyd0byb0H7cwexIBko0YgCSRSMZOp6oTQFUtKbKrxHDFiIbtgYSYNO5KW2BXW";
...
breedSelect.addEventListener('change', async () => {
  ...
  try {
      const response = await fetch(`https://api.thecatapi.com/v1/images/search?limit=5&breed_ids=${selectedBreedId}`, {
          headers: {
              "x-api-key": API_KEY
          }
      });
      ...
      breedInfo.forEach(info => {
          const imgSrc = info.url;
          const imgAlt = info.breeds[0].name;
          const imgId = info.id;
      ...
      infoDump.innerHTML = `
          ...
          <p>Life Span: ${breedInfo[0].breeds[0].life_span}</p>
      `;
  } catch (error) {
